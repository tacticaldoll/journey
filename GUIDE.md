...
## 0.1. Antigravity-Native Governance (GDD Protocol)

- **Antigravity-Native GDD (Guideline-Driven Development)**: This project's governance structure is **Antigravity-native**. It leverages the Antigravity agent system's `.agent/` protocols to enforce architectural rules. Under this **GDD** model, AI agents are empowered to act autonomously within these strict, machine-readable boundaries.
- **Native Enforcement Over Static Text**: Human-readable text in `GUIDE.md` is secondary to the executable boundaries defined in `.agent/skills/`. AI agents MUST strictly adhere to the active Antigravity `SKILL.md`. As long as operations remain within these safeguards, explicit `implementation_plan.md` approvals for daily tasks are NOT required.
- **Architectural Gatekeeping**: Major architectural shifts (e.g., modifying global SPA Router, adding new vendor dependencies, changing Hugo JSON compilation logic) fall outside standard GDD autonomy. These require explicit user approval before implementation.
...
*   **Typography & Animations**: Edit the CSS variables in the `:root` pseudo-class of `static/css/antigravity.css`. This file controls the "Antigravity Design System" (GDD) variables.
...
### AI Operational Ethics (Guideline-Driven Autonomy)
To ensure high development velocity while preventing architectural drift, AI agents are empowered with guarded autonomy:
1.  **Default to Action (GDD)**: For UI refinements, CSS tweaks, bug fixes, and non-structural feature additions, agents SHOULD directly implement and test the changes based on `SKILL.md` boundaries without pausing for an `implementation_plan.md` approval.
...
3.  **Prohibited "Surprise" Refactors**: Agents are STRICTLY FORBIDDEN from performing full-scale "consistency sweeps" or rewriting large files based on subjective design assumptions. All edits must be laser-focused on the user's specific request.
4.  **Automated Audit**: Agents MUST proactively use provided tools (like `/audit-codebase` or `verify-vendor`) after their changes to guarantee compliance, replacing manual pre-approval with automated post-verification.
...
Human-readable prose in `GUIDE.md` is passive. Complex rules MUST be transformed into **active enforcement perimeters** stored directly in the repository via the AI agent's native interface (`.agent/` directory):

1. **SKILLs (`.agent/skills/SKILL.md`) - Pre-computation Cognitive Boundaries**:
...
2. **Workflows (`.agent/workflows/*.md`) - Post-computation Audit & Automation Mcripts**:
...
If you introduce a complex new architectural pattern, you MUST NOT just document it here; you MUST extract it into an `.agent/skills/` package.
...
*   A utility script `.agent/scripts/download-vendor.py` is provided to automate the fetching and patching of all upstream dependencies.
*   A verification script `.agent/scripts/verify-vendor.py` is provided to check local vendor files against their remote sources to ensure they haven't been tampered with or corrupted.
...
*   **Strict CI/CD Separation**: Update (`download`) and assertion (`verify`) responsibilities MUST remain separated. CI/CD pipelines MUST ONLY run `python .agent/scripts/verify-vendor.py`.
*   **Mandatory Pre-Commit Hook**: To prevent architectural violations, all developers MUST install the pre-commit hook (`python .agent/scripts/install-hooks.py`).
