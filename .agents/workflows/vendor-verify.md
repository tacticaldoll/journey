...
1. Create a Python script named `scripts/verify-vendor.py`.
2. This script MUST:
   - Read the expected dependency list directly from `data/vendor.json` via `scripts/vendor_utils.py`. (Do not hardcode URLs).
...
3. Auto-run the script. (`python scripts/verify-vendor.py`) (// turbo)
...
## 2. CI/CD Integration
1. If requested by the user, integrate `python scripts/verify-vendor.py` into the project's CI/CD configurations (e.g., GitHub Actions, GitLab CI).
